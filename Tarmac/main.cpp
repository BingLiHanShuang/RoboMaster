...
#define blue
#ifdef blue
#define checkcolor checkblue
#endif

#ifdef red
#define checkcolor checkred
#endif
...
for (j = 0; j < width; j++)
{
CvScalar s_hsv = cvGet2D(hsv, i, j);//ªÒ»°œÒÀÿµ„Œ™£®j, i£©µ„µƒHSVµƒ÷µ
/*
opencv µƒH∑∂Œß «0~180£¨∫Ï…´µƒH∑∂Œß¥Û∏≈ «(0~8)°»(160,180)
S «±•∫Õ∂»£¨“ª∞„ «¥Û”⁄“ª∏ˆ÷µ,Sπ˝µÕæÕ «ª“…´£®≤Œøº÷µS>80)£¨
V «¡¡∂»£¨π˝µÕæÕ «∫⁄…´£¨π˝∏ﬂæÕ «∞◊…´(≤Œøº÷µ220>V>50)°£
*/
CvScalar s;
...
vector<CvBox2D> findcircle(IplImage* img, CvMemStorage* storage)
{
    ...
    IplImage* timg = cvCloneImage(img);//øΩ±¥“ª¥Œimg
    CvBox2D result;

    //±ﬂ‘µºÏ≤‚
    cvCanny(img, img_canny, 40, 120, 3);//±ﬂ‘µºÏ≤‚
    //cvShowImage("canny", img_canny);

    cvDilate(img_canny, img_dilate, NULL, 2); //≈Ú’Õ
    cvErode(img_dilate, img_erode, NULL, 2); //∏Ø ¥
    ...
    { //∂‡±ﬂ–Œ±∆Ω¸
        if (contours->total > 10)
        {
            ...
            sigm /= N;
            if (sigm <= 0.35 && result.size.height > 60 && result.size.height < 200 && result.size.width > 60 && result.size.width < 200)
...
int main()
{
    ...
    while (true)
    {
        ...
        cvClearMemStorage(storage);//«Âø’¥Ê¥¢
