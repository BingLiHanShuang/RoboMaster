...
void PadPassSend(uint8_t *digit_handwrite,uint8_t *digit_led){
    PadPass padPass=PAD_PASS__INIT;
    padPass.pad.data=digit_handwrite;
    padPass.pad.len=9;
    ...
    uint8_t buffer[100];
    data.len=pad_pass__pack(&padPass,buffer);
    data.data=buffer;


    MessageSend(MESSAGE__MESSAGE_TYPE__PadPass,data);
